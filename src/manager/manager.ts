...
export class RepositoryManager {
  ...
  public static getInstance(): RepositoryManager {
    if (!RepositoryManager.repositoryManager) {
  ...
  public getFileChangeFile(
    commitDetails: CommitDetails[],
    commitHash: string
  ): string | undefined {
    this.logger.debug(`getFileChangeFile(${commitHash})`);
    const commit = commitDetails.find(
    ...
    if (commit) {
      ...
      if (fileChange) {
        this.logger.debug(
          `getFileChangeFile(${commitHash}) = ${fileChange.file}`
        );
  ...
  public getExistingCommitDetails(
    repositoryPath: string,
    commitHash: string
  ): CommitDetails | undefined {
    this.logger.debug(
      `getExistingCommitDetails(${repositoryPath}, ${commitHash})`
    );
    ...
    if (commit) {
      this.logger.debug(
        `getExistingCommitDetails(${commitHash}) found ${commit}`
      );
      return commit;
    } else {
      this.logger.debug(
        `getExistingCommitDetails(${commitHash}) returned false`
      );
      return undefined; // Return false if commit is not found
    }
  }

  public noteDetailsExists(
    repositoryPath: string,
    commitHash: string
  ): boolean {
  ...
  public updateRepositoryDetails(
    repositoryPath: string | undefined,
    repositoryUrl: string,
    notes: CommitDetails[]
  ): void {
    this.logger.debug(
      `updateRepositoryDetails(${repositoryPath}, ${repositoryUrl}, notes found ${notes.length})`
    );
  ...
  public async updateNoteMessage(
    commitHashToUpdate: string,
    newNote: string,
    repositoryPath?: string | undefined
  ) {
    ...
    if (repoDetails) {
      const index = repoDetails.commitDetails.findIndex(
        (commit) => commit.commitHash === commitHashToUpdate
      );
      if (index !== -1) {
        this.logger.debug(
          `Note updated successfully for commit with hash ${commitHashToUpdate}`
        );
        repoDetails.commitDetails[index].note = newNote;
      } else {
        this.logger.debug(
          `Commit with hash ${commitHashToUpdate} not found. Note update failed.`
        );
      }
    }
  }

  public async clearRepositoryDetails(
  ...
  public async removeCommitByHash(
    commitHashToRemove: string,
    repositoryPath?: string | undefined
  ) {
    ...
    if (index !== -1) {
      this.repositoryDetailsInterface[index].commitDetails =
        this.repositoryDetailsInterface[index].commitDetails.filter(
          (commit) => commit.commitHash !== commitHashToRemove
        );
    }
  }

  public getGitRepositoryPath(
  ...
  public getRepositoryDetailsInterface() {
    this.logger.trace(
      `getRepositoryDetailsInterface(${this.repositoryDetailsInterface})`
    );
